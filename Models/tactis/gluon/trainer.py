...
from Models.tactis.gluon.utils import (
    save_checkpoint,
    load_checkpoint,
    set_seed,
)
...
all_batch_losses = []
all_batch_epochs = []
print("entereeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed")
    ...
    def __call__(
        ...
        if self.load_checkpoint:
            ...
            if ckpt["stage"] == 1:
                ...
                current_stage = 1
                #########################################################################################################################
                #optim.param_groups[0]['lr'] = self.learning_rate # <-- ADD THIS LINE
            else:
        ...
        if not self.skip_batch_size_search:
            ...
            for batch_no, data_entry in enumerate(tqdm(train_iter, total=self.training_num_batches_per_epoch), start=0):
        ...
        self.lr_scheduler = ReduceLROnPlateau(optim, 'min', patience=1, factor=0.5)
        ...
        while epoch_no < total_epochs:
            ...
            if switch_to_stage_2:
                ...
                self.lr_scheduler = ReduceLROnPlateau(optim, 'min', patience=2, factor=0.5)
...
###############################################
                ...
                # Estimate fractional epoch
                fractional_epoch = epoch_no + batch_no / self.training_num_batches_per_epoch

                # Print loss log
                
                all_batch_losses.append(loss_avg.item())
                all_batch_epochs.append(fractional_epoch)
...
#################################################################
            train_losses.append(avg_epoch_loss_unweighted)
            ...
            val_losses.append(avg_epoch_loss_val_unweighted)
            ...
            self.lr_scheduler.step(avg_epoch_loss_val_unweighted)
        ...
        for e, l in zip(all_batch_epochs, all_batch_losses):
            if l < 3:
                filtered_epochs.append(e)
                filtered_losses.append(l)

        plt.figure()
        plt.plot(filtered_epochs, filtered_losses, label='Filtered Batch Loss (<100)')
        plt.xlabel('Epoch (fractional)')
        plt.ylabel('Loss')
        plt.title('Filtered Training Loss Per Batch')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig("batch_loss_filtered.png")
        print("📊 Filtered per-batch loss plot saved as 'batch_loss_filtered.png'")




        # Save numerical log to text file
        with open("loss_log.txt", "w") as f:
            for i, (tr, vl) in enumerate(zip(train_losses, val_losses)):
                f.write(f"Epoch {i}: Train Loss = {tr:.6f}, Val Loss = {vl:.6f}\n")
        print("📝 Loss log saved to 'loss_log.txt'")

...
class TACTiS(nn.Module):
    ...
    def __init__(
        ...
        covariates_dim: int = 0,   # <-- NEW param, default 0
    ):
        """
        Parameters:
        -----------
        ...
        for i in range(self.flow_input_encoder_layers):
            if i == 0:
                flow_elayers.append(nn.Linear(self.flow_series_embedding_dim + 2 + self.covariates_dim, self.flow_encoder_embedding_dim))
            else:
            ...
            for i in range(self.copula_input_encoder_layers):
                if i == 0:
                    copula_elayers.append(
                        nn.Linear(self.copula_series_embedding_dim + 2 + self.covariates_dim, self.copula_encoder_embedding_dim)
    ...
    def loss(
        ...
        pred_time: torch.Tensor,
        pred_value: torch.Tensor,
        hist_covariates: torch.Tensor = None,   # [batch, series, hist_timesteps, d_cov]
        pred_covariates: torch.Tensor = None,   # [batch, series, pred_timesteps, d_cov]
    ) -> torch.Tensor:
        """
        ...
        if self.experiment_mode == "forecasting":
            ...
            all_values = torch.cat([hist_value, pred_value], dim=2)
            ...
            mask = torch.cat(
                [
                    torch.ones(num_batches, num_series, total_observed_timesteps_on_each_side, dtype=bool, device=device),
                    torch.zeros(num_batches, num_series, num_pred_timesteps, dtype=bool, device=device),
                    torch.ones(num_batches, num_series, total_observed_timesteps_on_each_side, dtype=bool, device=device),
                ],
                dim=2,
        ...
        if self.experiment_mode == "forecasting":
            ...
            if hist_covariates is not None:
                extra_hist.append(hist_covariates)
            hist_encoded_flow = torch.cat(extra_hist, dim=3)
            ...
            if pred_covariates is not None:
                extra_pred.append(pred_covariates)
            pred_encoded_flow = torch.cat(extra_pred, dim=3)
            ...
            if not self.skip_copula:
                ...
                if hist_covariates is not None:
                    extra_hist_copula.append(hist_covariates)
                hist_encoded_copula = torch.cat(extra_hist_copula, dim=3)
                ...
                if pred_covariates is not None:
                    extra_pred_copula.append(pred_covariates)
                pred_encoded_copula = torch.cat(extra_pred_copula, dim=3)

            # Encoding
            flow_encoded = torch.cat([hist_encoded_flow, pred_encoded_flow], dim=2)
            if not self.skip_copula:
                copula_encoded = torch.cat([hist_encoded_copula, pred_encoded_copula], dim=2)
            ...
            flow_encoded = self.flow_encoder.forward(flow_encoded)
            if not self.skip_copula:
                copula_encoded = self.copula_encoder.forward(copula_encoded)
    ...
    def sample(
        ...
        hist_value: torch.Tensor,
        pred_time: torch.Tensor,
        hist_covariates: torch.Tensor = None,
        pred_covariates: torch.Tensor = None,
    ) -> torch.Tensor:
        """
        ...
        hist_covariates: Tensor [batch, series, hist_timesteps, d_cov], optional
            Covariates aligned with history values.
        pred_covariates: Tensor [batch, series, pred_timesteps, d_cov], optional
            Covariates aligned with prediction horizon.
        ...
        # Expand over batches to be compatible with the bagging procedure
        copula_series_emb = None
        flow_series_emb = self.flow_series_encoder(torch.arange(num_series, device=device))
        if not self.skip_copula:
            copula_series_emb = self.copula_series_encoder(torch.arange(num_series, device=device))
        ...
        if self.experiment_mode == "forecasting":
            ...
            if not self.skip_copula:
                ...
                if hist_covariates is not None:
                    extra_hist_cop.append(hist_covariates)
                hist_encoded_copula = torch.cat(extra_hist_cop, dim=3)
                ...
                if pred_covariates is not None:
                    extra_pred_cop.append(pred_covariates)
                pred_encoded_copula = torch.cat(extra_pred_cop, dim=3)
            ...
            extra_hist = [
                ...
                torch.ones(num_batches, num_series, num_hist_timesteps - num_pred_timesteps, 1, device=device),
            ]
            if hist_covariates is not None:
                extra_hist.append(hist_covariates[:, :, :-num_pred_timesteps])
            hist_encoded_flow = torch.cat(extra_hist, dim=3)
            ...
            if not self.skip_copula:
                extra_hist_cop = [
                    observed_values[:, :, :, None],
                    copula_series_emb[:, :, None, :].expand(num_batches, -1, num_hist_timesteps - num_pred_timesteps, -1),
                    torch.ones(num_batches, num_series, num_hist_timesteps - num_pred_timesteps, 1, device=device),
                ]
                if hist_covariates is not None:
                    extra_hist_cop.append(hist_covariates[:, :, :-num_pred_timesteps])
                hist_encoded_copula = torch.cat(extra_hist_cop, dim=3)
